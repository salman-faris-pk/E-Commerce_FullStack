...
import { FilterOptions } from "@/app/collection/FilterOptions";

interface CollectionPageClientProps {
  initialProducts: productItemProp[];
}

export const CollectionPageClient = ({ initialProducts }: CollectionPageClientProps) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6">
          {isLoading && Allproducts?.length === 0 ? (
            Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="bg-gray-100 h-40 animate-pulse rounded"></div>
            ))
